...
# ================================================================
# ОСНОВНАЯ ФУНКЦИЯ ОЦЕНКИ BS (с фиксированным gamma)
# ================================================================
...
# ================================================================
# Оценка параметров BS с фиксированным gamma (численная)
# ================================================================

def fit_bs_fixed_loc_num(data: np.ndarray, censored: np.ndarray, fixed_loc: float) -> dict:
    """
    Оценка параметров BS распределения с фиксированным порогом gamma
...
# ================================================================
# АВТОМАТИЧЕСКИЙ ПОИСК ОПТИМАЛЬНОГО ПОРОГА gamma BS
# ================================================================
...
def plot_cdf_kaplan_meier_bs(results: dict):
    ...
    ax.set_title(f'Распределение Бирнбаума-Сандерса (оптимальный gamma = {loc:.0f})', fontsize=13)
...
def process_estimation_bs_anal() -> bool:
    
    inp_dir="Inp"
    out_dir="Out"
    inp_file="estimation_bs_anal.json"
    out_file="estimation_bs_anal.out" 
...
#=============================================================================
    ...
    if distr == "BS":  
        ...
        print("=" * 80,file=fout)
        print("ОЦЕНКА ПАРАМЕТРОВ РАСПРЕДЕЛЕНИЯ БИРНБАУМА-САНДЕРСА",file=fout)
        print("=" * 80,file=fout)
        ...
        print("=" * 80,file=fout)
        print(f"\nПОРОГ gamma = {results['optimal_gamma']:.2f} циклов",file=fout)
        ...
        print(f"  alpha={results['alpha']:.8f}",file=fout)
        print(f"  beta={results['beta']:.2f}",file=fout)
        print(f"  Медиана   = {results['median']:.2f} циклов",file=fout)
        ...
        print(f"  Ст.откл.  = {np.sqrt(var_val):.2f} циклов",file=fout)
        print(f"\nМИНИМИЗИРУЕМАЯ ФУНКЦИЯ:",file=fout)
        print(f"  Q = {results['Q_min']:.12e}",file=fout)
        print(f"\nСТАТИСТИКА ОПТИМИЗАЦИИ:",file=fout)
...
#====================Bootstrap======================================================
    ...
    if distr == "BS": 
        print("\n" + "=" * 80,file=fout)
        print("ПАРАМЕТРИЧЕСКИЙ БУТСТРЕП ДЛЯ BS-РАСПРЕДЕЛЕНИЯ",file=fout)
        print("=" * 80,file=fout)
        print(f"  Число бутстреп-итераций: 500",file=fout)
        print(f"  Доверительная вероятность: 95%",file=fout)
        bootstrap_results = []
